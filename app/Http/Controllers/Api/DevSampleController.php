...
class DevSampleController extends Controller
{
    public function show(): JsonResponse
    {
        ...
        $payload = $this->transform($legacy, $lookups);
        $this->anonymise($payload);
     ...
     * data so the test form never round-trips real personal data through the
     * UI. Landlord fields and free-text remarks are left as-is.
     */
    private function anonymise(array &$payload): void
    {
     ...
     */
    private function fetchLookups(): ?array
    {
        try {
    ...
    private function transform(array $l, array $lookups): array
    {
        ...
        $floorById = [
            '0' => 'Hochparterre', '1' => '1. Stock', '2' => '2. Stock',
            '3' => '3. Stock', '4' => '4. Stock', '5' => '5. Stock',
            '6' => '6. Stock',
        ];
        $relationshipById = [
            '1' => 'Ehepartner*in',
        ...
        $resolveNationality = function ($legacy) use ($lookups, $byLabel): ?string {
            if (!is_string($legacy) || $legacy === '') return null;
        ...
        $buildApplicant = function (string $prefix, bool $isMain) use ($l, $byLabel, $idToSlug, $resolveNationality, $maritalById, $employmentById, $incomeById, $rentDurById, $tenantRoleById, $bool01) {
            $a = [
                ...
                'current_housing' => [
                    ...
                    'rent_duration' => $idToSlug('rent_durations', $rentDurById, $l["{$prefix}_current_renter_rent_duration"] ?? null),
                    'previous_landlord' => $l["{$prefix}_current_renter_previous_renter"] ?? null,
                ],
            ];
        ...
        // Rooms in legacy payload are display labels like "4", "4&frac12;".
        // Decode the HTML entity, then label-match against the rooms lookup.
        $rooms = array_values(array_filter(array_map(
            function ($label) use ($byLabel) {
                if (!is_string($label)) return null;
                $decoded = html_entity_decode(str_replace('&frac12;', '½', $label), ENT_HTML5 | ENT_QUOTES, 'UTF-8');
                return $byLabel('rooms', $decoded);
            },
            (array) ($l['rent_pref_rooms_qty'] ?? [])
        )));
        ...
        $nobalconyYn = $bool01($l['rent_pref_nobalcony_yn'] ?? null);
        $noelevatorYn = $bool01($l['rent_pref_noelevator'] ?? null);
        ...
        return [
            ...
            'housing_wish' => [
                ...
                'max_gross_rent' => isset($l['rent_pref_max_rent']) && $l['rent_pref_max_rent'] !== ''
                    ...
                    : null,
                'wants_balcony' => $nobalconyYn === null ? null : !$nobalconyYn,
                'wants_elevator' => $noelevatorYn === null ? null : !$noelevatorYn,
                'districts' => $districts,
                'floors' => $floors,
                'rooms' => $rooms,
            ],
            'household_info' => [
                ...
                'plays_music' => $bool01($l['accomodation_play_music_yn'] ?? null) ?? false,
                'musical_instruments' => $l['accomodation_musical_instruments'] ?? null,
