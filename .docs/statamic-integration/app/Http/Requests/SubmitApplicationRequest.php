...
/**
 * Validates the public application form before forwarding to the Aporta backend.
 *
 * Strategy: validate shape, types, conditional requirements, and format here so
 * the visitor gets useful inline errors. Do NOT validate slug values against a
 * static list — slugs are owned by the backend and can be added/retired over
 * time. The backend re-validates everything regardless; an unknown slug there
 * surfaces as a 422 with the field path.
 *
 * Field names mirror the contract in Statamic-Integration.md §2.2.
 */
class SubmitApplicationRequest extends FormRequest
{
	public function authorize(): bool
	{
		// The visitor-facing password gate is enforced upstream (middleware on the page).
		return true;
	}
		...
		return array_merge(
			[
				...
				'household_info.pets_description' => [
					...
					'not_regex:/\b(Hund|Hunde|Dog)\b/i', // House rule "no dogs" — form-only.
				],
	...
	/**
	 * Normalize a phone string to E.164 using CH as the default region.
	 * Returns the original input on parse failure — validation will then accept
	 * it as a string, and the backend's strict normalization (giggle/libphonenumber)
